#' Rogers-Castro Nine Parameter Migration Schedule.
#'
#' Provides a scaled nine parameter Rogers-Castro schedule,
#' \deqn{ m(x) = a_{1} \exp[-\alpha_{1}x] +
#'   a_{2} \exp [ \alpha_{2}(x-\mu_{2})- \exp [ \lambda_{2}(x-\mu_{2}) ] ] +
#'   a_{4} \exp[\lambda_{4}x] +
#'   c}
...
#' @param a4 Numeric value for height of post-retirement curve.
#' @param lambda4 Numeric value for ascent of post-retirement curve.
...
#'
#' The parameters relate to the heights (\code{a1}, \code{a2}, \code{a4}, \code{c}), locations (\code{mu2}) and slopes (\code{alpha1}, \code{alpha2}, \code{lambda2}, \code{lambda4}).
#'
#' The default values of the parameters match the fundamental parameters of a simplified basic standard schedule migration schedule given in Rogers and Castro (1981). See examples below for non-zero parameters for the post-retirement slope.
#'
#'
...
#' @author Guy J. Abel
#' @seealso \code{\link{rogerscastro7}}, \code{\link{rogerscastro11}}, \code{\link{rogerscastro13}}
#' @examples
#' #single year
#' f0 <- rogerscastro9(tot_mig = 100, x = seq(0, 100, 1),
#'                      a1 = 0.02, alpha1 = 0.1,
...
#'
#' #five year
#' f1 <- rogerscastro9(tot_mig = 0.05, x = seq(0, 100, 5),
#'                      a1 = 0.02, alpha1 = 0.1,
...
#' plot(f1, type = "l")
#' sum(f1)
rogerscastro9 <- function(tot_mig = 1, x = seq(from = 0, to = 100, by = 1),
                           a1 = 0.02, alpha1 = 0.1,
                           a2 = 0.06, mu2 = 20, alpha2 = 0.1, lambda2 = 0.4,
                           a4 = 0, lambda4 = 0,
                           c = 0.03,
